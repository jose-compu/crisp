...
2. Failing that, it maps the `rustc` span back to the originating Crisp span via the CIR
   ...
   a crispc bug; please report it. (rustc said: <summary>)`
...
```
crispc build      -- analyze -> emit Rust -> invoke rustc
crispc run        -- build + run
crispc test       -- run tests
crispc check      -- analyze + emit, skip rustc codegen (fast)  (now: also typechecks emitted Rust with `cargo check`)
crispc emit       -- (new) emit Rust to target/rust/ and stop
reveal expand src/-- annotated Crisp for the project
reveal rust src/  -- (new) emitted Rust for the project
```

`crispc` invokes the system `rustc`/`cargo`; the emitted crate is an ordinary Cargo

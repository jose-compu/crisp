...
- **User-facing generics** (`type Pair<A, B>`, `id<T>(x: T)`, `trait Wrapper<T>`, applications `Pair<int, str>` / `Boxy<int>`): end-to-end in `examples/generics` ([#71](https://github.com/jose-compu/crisp/issues/71)). Syntax is `<>` (same as existing type applications). Remaining: `where` clauses, HRTB/GATs, rich `T: Show` bound emit.

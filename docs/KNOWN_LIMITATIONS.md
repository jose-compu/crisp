...
- **`shape`** data shapes work end-to-end for field accessors (`examples/shapes`): generated trait + structural call sites (§3.5). Remaining gaps: method shapes, anonymous `{ x: float }` params, full bound/`dyn`-style use ([#61](https://github.com/jose-compu/crisp/issues/61)); parametric shapes ([#70](https://github.com/jose-compu/crisp/issues/70)).
...
- **User-facing generics** (explicit type/function params): incomplete; track [#71](https://github.com/jose-compu/crisp/issues/71) (v1.6.0).

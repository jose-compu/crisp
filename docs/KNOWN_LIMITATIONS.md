...
- **Closures / function values** (spec §5.2–§5.3): design target is **one** callable kind (closure); naming optional. Parsed / partially typed today; **not** emitted end-to-end — [#72](https://github.com/jose-compu/crisp/issues/72) (v1.6.0).
- **User-facing generics** (`type Pair<A, B>`, `id<T>(x: T)`, `trait Wrapper<T>`, applications `Pair<int, str>` / `Boxy<int>`): end-to-end in `examples/generics` and `examples/shapes_generic` ([#71](https://github.com/jose-compu/crisp/issues/71)). **Free type names** are binders without `<T>` (`id(x: T)`, `type Pair = { left: A, right: B }`, `shape Boxy = { value: T }`) — `examples/generics_implicit` ([#75](https://github.com/jose-compu/crisp/issues/75)). `impl Wrapper for IntBox` infers `Wrapper<int>` from the body; `item: HasName + HasId` emits extra Rust bounds ([#77](https://github.com/jose-compu/crisp/issues/77)). Typeck checks the first (inner) constraint; further `+` bounds are enforced by `rustc`. In-scope types stay types; `<T>` shadowing a type is E0049 ([#78](https://github.com/jose-compu/crisp/issues/78)). `<>` remains a pin. Remaining: crate-wide generalization ([#76](https://github.com/jose-compu/crisp/issues/76)), anonymous `{ value: T }` params, `where` / HRTB/GATs. Nested applications that end in `>>` (`Pair<T, Pair<A, B>>`) are lexed as a shift token — write a space (`> >`) or avoid nesting for now.

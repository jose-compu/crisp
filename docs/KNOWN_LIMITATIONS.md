# Known limitations (Crisp v1.6.x / unreleased v1.7.0)
...
- **Function values** (spec §5.2–§5.3): one callable kind — named items and `|x| …` are the same. Holes (`_ * 2`), trailing last-arg (`run { |x| … }`), field sections (`.name`) — `examples/closures` (#72, #87–#89). Remaining: operator sections (`+ 1`), `dyn Fn`.

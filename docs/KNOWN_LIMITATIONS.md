...
- **Loops (`while` / `for` / `loop`):** work end-to-end (`examples/loops`), including `break` / `continue`, value-producing `break <expr>` on `loop`, assignment in `if then` / `else` inside `while` (float bisection, [#96](https://github.com/jose-compu/crisp/issues/96)), and chained `else if` (then-form and brace-form, [#117](https://github.com/jose-compu/crisp/issues/117)). `for` iterates Crisp `vec<T>` (`.iter().copied()` when `T` is Copy, else `.cloned()`); no `enumerate`, labels, or general `IntoIterator` yet. `if`/`while`/`for` heads disallow bare struct literals so `{` starts the body (parenthesize struct lits in conditions when needed).
...
- **`test "name"`** becomes `fn test_<module>_<sanitized>` so titles can repeat across modules ([#102](https://github.com/jose-compu/crisp/issues/102)). Float `assert_eq` uses a small epsilon and parenthesizes the RHS (`(left - (right)).abs() < 1e-9`, [#113](https://github.com/jose-compu/crisp/issues/113)). Bool/str/comparisons stay `assert_eq!`. Nested user traits emit `pub`. Call arguments use CIR ownership modes, matching production emit ([#114](https://github.com/jose-compu/crisp/issues/114)). Crisp string literals with unescaped quotes/`\` can still break harness emit ([#17](https://github.com/jose-compu/crisp/issues/17)).
...
- **Shipped:** `vec<T>` via `new` / `push` / `len` / `[1.0, 2.0]` and `xs[i]` ([#119](https://github.com/jose-compu/crisp/issues/119), [#120](https://github.com/jose-compu/crisp/issues/120)), prelude `exp` / `sin` / `cos` / `tanh` / `sqrt` ([#115](https://github.com/jose-compu/crisp/issues/115)), limited fs/async, prelude Show/Eq/Ord ([#27](https://github.com/jose-compu/crisp/issues/27)), `std.net.parse_ip`, thin HTTP via `ureq` ([#28](https://github.com/jose-compu/crisp/issues/28)), `**` (`.powf`).
- **Not shipped:** `ln` / `log` as math (prelude `log` is print), `tan` / `abs` / `floor`, full Crisp `std.http` server API (§20), channels ([#38](https://github.com/jose-compu/crisp/issues/38) → v2.0.0), most of the §15 trait catalog beyond Show/Eq/Ord. Typed `extern rust` scalars are [#116](https://github.com/jose-compu/crisp/issues/116).

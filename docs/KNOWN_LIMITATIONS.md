...
- **User-facing generics** (`type Pair<A, B>`, `id<T>(x: T)`, `trait Wrapper<T>`, applications `Pair<int, str>` / `Boxy<int>`): end-to-end in `examples/generics` and `examples/shapes_generic` ([#71](https://github.com/jose-compu/crisp/issues/71)). **Free type names** are binders without `<T>` (`id(x: T)`, `type Pair = { left: A, right: B }`, `shape Boxy = { value: T }`) — `examples/generics_implicit` ([#75](https://github.com/jose-compu/crisp/issues/75), thin [#77](https://github.com/jose-compu/crisp/issues/77)). In-scope types stay types; `<T>` shadowing a type is E0049 ([#78](https://github.com/jose-compu/crisp/issues/78)). `<>` remains a pin. Remaining: crate-wide generalization ([#76](https://github.com/jose-compu/crisp/issues/76)), `where` / HRTB/GATs, rich `T: Show` bound emit. Nested applications that end in `>>` (`Pair<T, Pair<A, B>>`) are lexed as a shift token — write a space (`> >`) or avoid nesting for now.

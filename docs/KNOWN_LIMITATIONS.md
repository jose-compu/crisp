...
This page documents behaviors that surprise users and are **not** always full language bugs. Formal deltas vs the draft spec: [SPEC_IMPL_DELTA.md](SPEC_IMPL_DELTA.md). Open follow-ups: crates.io ([#66](https://github.com/jose-compu/crisp/issues/66)), visibility ([#58](https://github.com/jose-compu/crisp/issues/58)), trait bounds/`dyn` ([#59](https://github.com/jose-compu/crisp/issues/59)), collections ([#110](https://github.com/jose-compu/crisp/issues/110)), `else if` ([#117](https://github.com/jose-compu/crisp/issues/117)), record `:=` moves ([#118](https://github.com/jose-compu/crisp/issues/118)).
...
- **Loops (`while` / `for` / `loop`):** work end-to-end (`examples/loops`), including `break` / `continue`, value-producing `break <expr>` on `loop`, and assignment in `if then` / `else` inside `while` (float bisection, [#96](https://github.com/jose-compu/crisp/issues/96)). **`else if` does not parse** (then-form or brace-form): `else if` is lowered as if the inner `if` were the *condition*. Write `else { if … }` ([#117](https://github.com/jose-compu/crisp/issues/117)). `for` currently iterates Crisp `vec` only (emits `.iter().copied()`); no `enumerate`, labels, or general `IntoIterator` yet. `if`/`while`/`for` heads disallow bare struct literals so `{` starts the body (parenthesize struct lits in conditions when needed).
...
- **Record literals** are newline-separated fields. A comma is a parse error (`Point { x: 1.0, y: 2.0 }` fails; write each field on its own line). Spec examples use commas ([#111](https://github.com/jose-compu/crisp/issues/111)).
